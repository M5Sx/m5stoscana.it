import { getAllPressReleases, getPressRelease, getAdjacentPressReleases } from "@/lib/pressreleases";
...
export async function generateStaticParams() {
  return getAllPressReleases().map((post) => ({ slug: post.slug }));
}
...
export async function generateMetadata({
  ...
  return { title: post?.title ?? "Comunicato Stampa" };
}
...
export default async function PressReleaseDetailPage({
  ...
  const post = await getPressRelease(slug);
  if (!post) notFound();
  const { prev, next } = getAdjacentPressReleases(slug);
  ...
  const dateStr = post.date
    ...
    : "";
  const dateStrCapitalized = dateStr ? dateStr.charAt(0).toUpperCase() + dateStr.slice(1) : "";
